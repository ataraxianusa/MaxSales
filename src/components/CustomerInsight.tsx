  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* ====== LEFT 2 COLUMNS: Segments + Form + CRM Rules + AI Tools ====== */}
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {segments.map((seg, idx) => (
              <div key={idx} className="p-5 rounded border bg-white dark:bg-[#111111] border-neutral-200 dark:border-[#262626] transition-all flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[9px] font-mono text-neutral-800 dark:text-[#E5E5E5] bg-neutral-100 dark:bg-[#262626] px-2 py-0.5 rounded uppercase tracking-wider">Segmentasi {seg.percentage}% Pasar</span>
                    <button onClick={() => deleteSegment(idx)} className="text-[10px] text-red-500 hover:underline">Hapus</button>
                  </div>
                  <h4 className="text-xs font-bold text-neutral-900 dark:text-white mb-1">{seg.name}</h4>
                  <p className="text-[11px] text-neutral-500 dark:text-[#A3A3A3] mb-4">Kanal Utama: <strong className="text-neutral-700 dark:text-neutral-300">{seg.channel}</strong></p>
                ...
                <div className="mt-4 pt-3 border-t border-neutral-100 dark:border-[#262626] flex items-center justify-between text-[11px]">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[9px] font-mono border uppercase tracking-wider ${getRiskBadgeColor(seg.risk)}`}>{seg.risk} Risk</span>
                </div>
              </div>
            ))}
          </div>

          {/* Form: Petakan Segmen Pelanggan */}
          <div className="p-5 rounded border bg-white dark:bg-[#111111] border-neutral-200 dark:border-[#262626] space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
              <div>
                ...
                <input id="inp-seg-name" type="text" placeholder="misal: Anggota Komunitas Ibu Arisan" value={segName} onChange={e => setSegName(e.target.value)} className="w-full text-xs px-3 py-2 rounded border bg-transparent border-neutral-300 dark:border-neutral-800 focus:border-neutral-900 dark:focus:border-[#E5E5E5] focus:outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-mono font-semibold text-neutral-450 dark:text-[#737373] mb-1">% Pangsa Pasar</label>
                <input id="inp-seg-percent" type="number" min={1} max={100} value={segPercent} onChange={e => setSegPercent(Number(e.target.value))} className="w-full text-xs px-3 py-2 rounded border bg-transparent border-neutral-300 dark:border-neutral-800 focus:border-neutral-900 dark:focus:border-[#E5E5E5] focus:outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-mono font-semibold text-neutral-450 dark:text-[#737373] mb-1">Kanal Utama</label>
                <select id="inp-seg-channel" value={segChannel} onChange={e => setSegChannel(e.target.value)} className="w-full text-xs px-3 py-2 rounded border bg-transparent border-neutral-300 dark:border-neutral-800 focus:border-neutral-900 dark:focus:border-[#E5E5E5] focus:outline-none">
                  {["WhatsApp DM", "Instagram DM", "Shopee Chat", "TikTok Shop", "Toko Fisik", "Marketplace Lain"].map(ch => <option key={ch} value={ch}>{ch}</option>)}
                </select>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div>
                <label className="block text-[10px] font-mono font-semibold text-neutral-450 dark:text-[#737373] mb-1">Rerata Transaksi (Rp)</label>
                <input id="inp-seg-spend" type="number" value={segAvgSpend} onChange={e => setSegAvgSpend(Number(e.target.value))} className="w-full text-xs px-3 py-2 rounded border bg-transparent border-neutral-300 dark:border-neutral-800 focus:border-neutral-900 dark:focus:border-[#E5E5E5] focus:outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-mono font-semibold text-neutral-450 dark:text-[#737373] mb-1">Resiko Churn</label>
                <select id="inp-seg-risk" value={segRisk} onChange={e => setSegRisk(e.target.value as "Low" | "Medium" | "High")} className="w-full text-xs px-3 py-2 rounded border bg-transparent border-neutral-300 dark:border-neutral-800 focus:border-neutral-900 dark:focus:border-[#E5E5E5] focus:outline-none">
            ...
            <button id="btn-add-segment" onClick={addNewSegment} className="w-full py-2 rounded border border-neutral-350 dark:border-[#262626] text-xs font-mono hover:bg-neutral-50 dark:hover:bg-[#1A1A1A] text-neutral-700 dark:text-[#A3A3A3] flex items-center justify-center space-x-1">
              <Plus className="w-3.5 h-3.5" />
              <span>Tambah Segmen</span>
            </button>
          </div>

          {/* CRM COUNTER-CHURN CAMPAIGN RULES */}
          <div className="p-5 rounded border bg-neutral-50 dark:bg-[#111111] border-neutral-200 dark:border-[#262626] text-neutral-800 dark:text-white space-y-3.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
              <div className="p-3.5 rounded border border-neutral-200 dark:border-[#262626] bg-white dark:bg-[#1A1A1A] space-y-1">
                <strong className="text-neutral-900 dark:text-white block font-bold font-mono text-[10px]">1. PENYELAMATAN SEGMEN HIGH-RISK</strong>
                <p className="text-neutral-500 dark:text-[#A3A3A3] leading-relaxed text-[11px]">Mereka sensitif pada margin harga. Berikan kupon diskon volume instan flat-rate senilai <strong>15%</strong> jika tidak bertransaksi dalam 45 hari.</p>
              </div>
              <div className="p-3.5 rounded border border-neutral-200 dark:border-[#262626] bg-white dark:bg-[#1A1A1A] space-y-1">
                <strong className="text-neutral-900 dark:text-white block font-bold font-mono text-[10px]">2. PENGIKATAN SEGMEN LOW-RISK</strong>
                <p className="text-neutral-500 dark:text-[#A3A3A3] leading-relaxed text-[11px]">Berikan tawaran unboxing experience premium gratis cetak foto kartu ucapan personal, poin voucher loyalty bulanan, dan prarilis eksklusif.</p>
              </div>
            </div>
          </div>
        ...
        {/* ====== RIGHT 1 COLUMN: LTV Calculator (full height) ====== */}
        <div className="lg:col-span-1">
          <div className="p-6 rounded border bg-white dark:bg-[#111111] border-neutral-200 dark:border-[#262626] space-y-6 lg:sticky lg:top-6">
            ...
            <div className="space-y-2">
              ...
              <input id="slide-ltv-aov" type="range" min={50000} max={1500000} step={25000} value={aov} onChange={e => setAov(Number(e.target.value))} className="w-full accent-neutral-950 dark:accent-white cursor-pointer" />
              <p className="text-[9px] text-neutral-400 leading-none">Rata-rata nominal pengeluaran sekali belanja.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input id="slide-ltv-freq" type="range" min={1} max={10} step={1} value={frequency} onChange={e => setFrequency(Number(e.target.value))} className="w-full accent-neutral-950 dark:accent-white cursor-pointer" />
              <p className="text-[9px] text-neutral-400 leading-none">Berapa kali pelanggan membeli dalam sebulan.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <input id="slide-ltv-lifespan" type="range" min={1} max={36} step={1} value={lifespan} onChange={e => setLifespan(Number(e.target.value))} className="w-full accent-neutral-950 dark:accent-white cursor-pointer" />
              <p className="text-[9px] text-neutral-400 leading-none">Keawetan hubungan sebelum pelanggan churn.</p>
            </div>
            ...
            <div className="p-4 rounded bg-[#111111] dark:bg-[#171717] border border-[#262626] text-center space-y-1.5">
              ...
              <p className="text-lg sm:text-xl font-bold font-mono text-white dark:text-[#E5E5E5]">Rp {ltvResult.toLocaleString("id-ID")}</p>
              <p className="text-[10px] text-neutral-400 leading-normal px-2">Setiap 1 pembeli bernilai ekonomis kotor sebesar ini bagi bisnis Anda selama masa retensinya!</p>
            </div>
            ...
            <div className="text-[10px] text-neutral-450 leading-normal p-3 rounded border border-dashed border-neutral-200 dark:border-[#262626] bg-neutral-50 dark:bg-[#111111] text-center">
              💡 <strong className="font-semibold text-neutral-950 dark:text-white font-mono">Matematika Bisnis:</strong> Anda boleh beriklan aman (CAC) hingga <strong>Rp {(ltvResult*0.25/1000).toFixed(0)}rb</strong> per pembeli baru.
            </div>
          </div>
        </div>
