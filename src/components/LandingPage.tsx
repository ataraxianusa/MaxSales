...
function SectionLabel({ n, label }: { n: number; label: string }) {
  return (
    ...
    >
      <span className="font-mono text-xs tracking-[0.15em] text-[#006241] dark:text-[#006241] font-bold">
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-5">
        ...
        <span className="font-mono text-[10px] tracking-[0.1em] text-[#006241] dark:text-[#006241]">
          HARI INI
        </span>
      </div>
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-2">
          <Target className="w-3.5 h-3.5 text-[#006241]" />
        ...
        <p className="mt-2 font-mono text-xs text-[#006241] dark:text-[#006241]">
      ...
      <div className="mb-5">
        <div className="flex items-center gap-2 mb-3">
          <Zap className="w-3.5 h-3.5 text-[#cba258]" />
        ...
        <div className="space-y-2.5">
          {["Cek DM WhatsApp jam 09:00 — balas personal", "Hubungi 5 pelanggan repeat-order", "Foto 3 varian produk untuk story besok"].map((step, i) => (
            <div key={i} className="flex gap-3">
              <span className="font-mono text-[10px] text-[#006241] dark:text-[#006241] font-bold mt-0.5 shrink-0">{String(i + 1).padStart(2, "0")}</span>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <MessageSquare className="w-3.5 h-3.5 text-[#1E3932]" />
...
export default function LandingPage({ onEnterDashboard }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-base dark:bg-space-dark text-ink dark:text-dark-text font-body transition-colors duration-300">
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          <div>
            <motion.h1 className="font-display font-extrabold text-4xl md:text-5xl lg:text-6xl text-ink dark:text-dark-text mb-6 leading-tight" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              Ubah Produk Biasa<br /><span className="text-[#006241] dark:text-[#00754A]">Jadi Luar Biasa</span>
            </motion.h1>
            ...
            <motion.div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-ink/40 dark:text-dark-text/40 font-mono" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.5 }}>
              <span className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-[#00754A]" /> Lifetime access</span>
              <span className="flex items-center gap-1.5"><Check className="w-3.5 h-3.5 text-[#00754A]" /> Tanpa biaya tersembunyi</span>
              <span className="flex items-center gap-1.5"><Zap className="w-3.5 h-3.5 text-[#cba258]" /> Siap pakai 5 menit</span>
            </motion.div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {PROBLEMS.map((p, i) => (
              <motion.div key={i} className="card-sb p-6 bg-white dark:bg-charcoal-surface" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <div className="w-10 h-10 flex items-center justify-center border border-[#006241]/20 mb-4">
                  <p.icon className="w-5 h-5 text-[#006241]" />
                </div>
      ...
      {/* ── FEATURES ──────────────────────────────────── */}
      <section className="border-t border-ink/10 dark:border-stone-border bg-[#edebe9]/50 dark:bg-dark-krem/30">
        <div className="max-w-6xl mx-auto px-6 md:px-12 py-16 md:py-24">
            ...
            {FEATURES.map((f, i) => (
              <motion.div key={i} className="card-sb p-6 bg-white dark:bg-charcoal-surface group" initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-30px" }} transition={{ duration: 0.4, delay: i * 0.06 }}>
                <div className="flex items-center gap-3 mb-3">
                  <div className={`w-8 h-8 flex items-center justify-center border ${f.color === "merah" ? "border-[#006241]/20" : f.color === "emas" ? "border-[#cba258]/20" : "border-[#1E3932]/20"}`}>
                    <f.icon className={`w-4 h-4 ${f.color === "merah" ? "text-[#006241]" : f.color === "emas" ? "text-[#cba258]" : "text-[#1E3932]"}`} />
                  </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {STEPS.map((s, i) => (
              ...
              >
                {/* Big number */}
                <motion.div
                  className="w-14 h-14 rounded-full flex items-center justify-center border-2 border-[#00754A] dark:border-[#00754A] mb-5 bg-white dark:bg-charcoal-surface transition-all duration-300 group-hover:bg-[#00754A]"
                  whileHover={{ scale: 1.05 }}
                >
                  <span className="font-body text-xl font-extrabold text-[#00754A] group-hover:text-white transition-colors duration-300">{s.n}</span>
                </motion.div>
      ...
      {/* ── PRICING ───────────────────────────────────── */}
      <section id="pricing" className="border-t border-ink/5 dark:border-dark-text/5 bg-[#edebe9]/50 dark:bg-dark-krem/30">
        <div className="max-w-6xl mx-auto px-6 md:px-12 py-16 md:py-24">
            ...
            {TESTIMONIALS.map((t, i) => (
              ...
              >
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 rounded-full bg-[#1E3932]/10 dark:bg-[#1E3932]/20 flex items-center justify-center">
                    <span className="font-mono text-xs font-bold text-[#1E3932] dark:text-[#1E3932]">{t.initials}</span>
                  </div>
                  <div>
      ...
      {/* ── FAQ ────────────────────────────────────────── */}
      <section id="faq" className="border-t border-ink/5 dark:border-dark-text/5 bg-[#edebe9]/50 dark:bg-dark-krem/30">
            ...
            {FAQ_ITEMS.map((item, i) => (
              <motion.div key={i} className="border-b border-ink/10 dark:border-stone-border" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05 }}>
                <button className="w-full flex items-center justify-between py-5 text-left group cursor-pointer" onClick={() => setOpenFaq(openFaq === i ? null : i)}>
                  <span className="font-body font-bold text-sm text-ink dark:text-dark-text group-hover:text-[#00754A] dark:group-hover:text-[#00754A] transition-colors pr-4">{item.q}</span>
      ...
      {/* ── Footer ────────────────────────────────────── */}
      <footer className="bg-[#1E3932] border-t border-stone-border py-12 text-white">
        <div className="max-w-6xl mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-6 text-xs font-mono text-white/70">
            <Link to="/about" className="hover:text-[#cba258] transition-colors">Tentang</Link>
            <Link to="/terms" className="hover:text-[#cba258] transition-colors">Syarat</Link>
            <Link to="/privacy" className="hover:text-[#cba258] transition-colors">Privasi</Link>
            <Link to="/risk" className="hover:text-[#cba258] transition-colors">Risiko</Link>
          </div>
        </div>
      </footer>
    </div>
